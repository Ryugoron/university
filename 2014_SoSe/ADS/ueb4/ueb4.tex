...
Dies bedeutet für uns, dass wir die folgenden Wahrscheinlichkeiten berechnen müssen für ein Intervall der Länge $m$.
\begin{description}
	\item[$\prob{\pi_i < \pi_j}$:] Als Wahrscheinlichkeit erhalten wir alle Permuatationen, bei denen $i$ in der ersten Hälfte und $j$ in der zweiten
		Hälfte steht.
		$$\begin{array}{rcl}
			\prob{\pi_i < \pi_j} &=& \left( \frac{1}{(m/2)!} \right)^2 \geq \frac{1}{m^2}
		\end{array}$$
	\item[$\prob{j \not= lca(i,j)}$:] Dies ist nun erheblich leichter, da wir uns auf dem Intervall der Länge $m$ befinden. Für dieses
		wissen wir, dass nur ein Element. Auf für alle $j$ auf dem Intervall gibt es also nur ein einziges, welches der lca sein könnte.
		$\Rightarrow \prob{...} \geq \frac{1}{m}$.
	\item[$\prob{\forall k \in u(lca(i,j)) \, : \, \pi_j \leq \pi_i \lor \pi_k \geq \pi_j}$:] Hier ist wichtig,
		wie wir das ganze aufschreiben. Die Wahrscheinlichkeit für eine Permutation ist ein Zufallsexperiment von Positionen für die einzelnen 		
		Elemente ohne zurücklegen. Was wir also tun, ist, dass wir zunächst allen Elemente, die sonst noch in diesem Intervall liegen können
		andere Positionen außerhalb von $\pi_i$ und $\pi_j$ zuweisen. Der Fall, bei dem es hier am kleinsten wird ist, wenn $i$,$j$ ganz am Rand
		des Intervalls liegen, da wir so weniger Möglichkeiten haben um alles zu ersetzen. Sei $m = n/2^i$ (wir wissen, das es so gebildet wird).
		$$\begin{array}{rcl}
			\prob{\forall k \in u(lca(i,j)) \, : \, \pi_j \leq \pi_i \lor \pi_k \geq \pi_j} &\geq& 
				\sum_{s=0}^{m-2} \frac{1}{n-m-s}\\
			&=& \sum_{s=n-m}^{n-2} \frac{1}{s}\\
			&=& H_{n-2} - H_{n-m}\\
			&=& \Omega( \log (n - 2) - \log (n - m))\\
			&=& \Omega(\log (n-2) - \log (n - n/2^i))\\
			&=& \Omega(\log (n-2) - \log ((2^i-1)n / 2^i))\\
			&=& \Omega(\log (n-2) - \log (2^i - 1) - \log \, n + \log \, 2^i)\\
			&=& \Omega(\log \, 2^i - \log (2^i - 1))\\
			&=& \Omega (\log \, \frac{2^i}{2^i - 1})\\
			&=& \Omega(\log 1/m)
