...
\noindent{textbf{Beweis \ref{ads:ueb2:lemma2}.}\\
...
Um den Satz von Shannon anwenden zu können müssen wir nun einen prefixfreien Code finden, der in mit der Tiefe des Angefragen Knotens
korreliert. Wir konstruieren den Code wie folgt:\\

Seien $d(x)$ die Tiefe für Knoten $x$ im ursprünglichen Baum und $p(x)$ die binärcodierung der Zahl die beschreibt, der wie vielte Knoten $x$ in seiner Ebene ist. Wobei alle Knoten in tiefe $d(x)$ genau $d(x)$ Bits benutzen um $p(x)$ zu Codieren.\\
