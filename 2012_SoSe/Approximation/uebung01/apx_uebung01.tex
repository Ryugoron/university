...
Let $n =|V|$ the amount of vertices, $i = \{ v \in V \; | \; \deg (v) \geq 2 \}$ the inner amount of inner vertices.\\

Because every vertex excpet for the root in the tree has exactly one parent. So the amount of tree edges is $n-1$ and the amount of edges in between the nonleaf vertices is $i-1$.\\
...
Considering that
$$i \leq 2 \cdot i \leq 2 \cdot \frac{i - 1}{2} + \frac{1}{2} \leq 2 \cdot OPT + \frac{1}{2}$$ holds, 
but both the optimal and the feasible solution of $\mathfrak{D}$ are natural Numbers, so the equation
$$
i \leq 2 \times OPT
$$
holds. Now we can conclude that
$$
|C| \leq 2 \times OPT
$$
